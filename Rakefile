...
task :build => [:clean] do
  ...
  # copy src
  cp 'jquery.safetynet.js', 'dist/jquery.safetynet.js'
  ...
  # minify src
  source = File.read('dist/jquery.safetynet.js')
  minified = Packr.pack(source, :shrink_vars => true, :base62 => false)
  header = /\/\*.*?\*\//m.match(source)

  # inject header
  File.open('dist/jquery.safetynet.min.js', 'w') do |combined|
...
task :release => [:build] do
  root = pwd+'/dist'
  zip_archive = pwd+'/dist/jquery.safetynet.zip'
...
task :test => [:build] do  
  begin
    # mac
    sh("open spec/jquery.safetynet.specs.html")
  rescue
    # windows
    sh("start spec/jquery.safetynet.specs.html")
  end
end
